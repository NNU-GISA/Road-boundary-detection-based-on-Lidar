...
road_segmentation::road_segmentation(PointCloudXYZI::Ptr incloud)

{
    _completeCloud.reset(new PointCloudXYZI);
...
void road_segmentation::computeDistanceVec()
{
#pragma omp parallel for  schedule(runtime)
    for (int i = 0; i < _grid_map_vec.size(); ++i)
    {
        sort(_grid_map_vec[i].begin(),_grid_map_vec[i].end(),[](PointT& left,PointT& right)
        {
            double d1=pow(pow(left.x,2)+pow(left.y,2),0.5);
            double d2=pow(pow(right.x,2)+pow(right.y,2),0.5);
            return(d1<d2);
        });
        ...
        if (_grid_map_vec[i].size()>0)
        {
            ...
            double distance_max=pow(pow(_grid_map_vec[i][_grid_map_vec[i].size()-1].x,2)+
                    pow(_grid_map_vec[i][_grid_map_vec[i].size()-1].y,2),0.5);
            _distance_vec[i]=pair<double,int>(distance_min/distance_max,i);
            ...
            //            if(i>90&&i<270)
            //            {
            //                _distance_vec_rear.push_back(pair<double,int>(distance_min/distance_max,i));
            //            }
            //            else
            //            {
            //                _distance_vec_front.push_back(pair<double,int>(distance_min/distance_max,i));
            //            }
        }
        else
        {
            ...
            point.z=0;
            _distance_vec[i]=pair<double,int>(1,i);
            _distance_vec_[i]=1;
            _nearest_points[i]=point;
            //            if(i>90&&i<270)
            //            {
            //                _distance_vec_rear.push_back(pair<double,int>(1,i));
            //            }
            //            else
            //            {
            //                _distance_vec_front.push_back(pair<double,int>(1,i));
            //            }
        }
...
//            vector<double> temp;
        ...
        if (i>90&&i<270)
        {
            _distance_vec_rear.push_back(pair<double,int>(temp[2],i));
        }
        else
        {
            _distance_vec_front.push_back(pair<double,int>(temp[2],i));
        }
...
void road_segmentation::computeSegmentAngle()
{
    sort(_distance_vec_front.begin(),_distance_vec_front.end(),[](pair<double,int>& left,pair<double,int>& right)
    {
        return left.first>right.first;
    });
    sort(_distance_vec_rear.begin(),_distance_vec_rear.end(),[](pair<double,int>& left,pair<double,int>& right)
    {
        return left.first>right.first;
    });

    //计算前方segmentation angle
    if(_distance_vec_front[0].first==1)
    {
        ...
        {
            if (_distance_vec_front[i].first==1&&_distance_vec_front[i].second<90)
            {
                max_distance_angle_left.push_back(_distance_vec_front[i].second);
            }
            if (_distance_vec_front[i].first==1&&_distance_vec_front[i].second>270)
            {
    ...
    if(_distance_vec_rear[0].first==1)
    {
    ...
    cout<<"前方分割角度是 "<<_segmentAngle[0]<<endl;
    cout<<"后方分割角度是 "<<_segmentAngle[1]<<endl;
}


void road_segmentation::process(PointCloudXYZI::Ptr incloud, vector<PointCloudXYZI::Ptr> outcloud)
{
