                  ...
                  double* distanceLeft_,double* distanceRight_)
{
    ...
    while(i<reader.getNumberofFile())
    {
        ...
        if(update==false)
        {
            ...
            ground_segment ground(completeCloud);
            ...
            refinePoints.process(ground_points,clusterPtr);
            //             左右curb点拟合
            ...
            *distanceLeft_=abs((0-(*lineLeft3D)[0].x)/((*lineLeft3D)[1].x-(*lineLeft3D)[0].x)*((*lineLeft3D)[1].y-(*lineLeft3D)[0].y)+(*lineLeft3D)[0].y);
            *distanceRight_=abs((0-(*lineRight3D)[0].x)/((*lineRight3D)[1].x-(*lineRight3D)[0].x)*((*lineRight3D)[1].y-(*lineRight3D)[0].y)+(*lineRight3D)[0].y);
//cout<<"Right curb distance is "<<distanceRight<<" m"<<endl;
...
int main(int argc,char** argv)
{
    ...
    while (!viewer.wasStopped())
    {
...

        {
            ...
            viewer.addLine(lineLeft3D[0],lineLeft3D[1],"lineleft",v1);
            viewer.addLine(lineRight3D[0],lineRight3D[1] ,"lineright",v1);
            viewer.setShapeRenderingProperties(pcl::visualization::PCL_VISUALIZER_LINE_WIDTH,3,"lineleft",v1);
            viewer.setShapeRenderingProperties(pcl::visualization::PCL_VISUALIZER_LINE_WIDTH,3,"lineright",v1);
            viewer.setShapeRenderingProperties(pcl::visualization::PCL_VISUALIZER_COLOR,0,1,0,"lineright",v1);
            viewer.setShapeRenderingProperties(pcl::visualization::PCL_VISUALIZER_COLOR,0,1,0,"lineleft",v1);
